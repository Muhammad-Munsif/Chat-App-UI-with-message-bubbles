  <script>
    (function () {
      ...
      const conversations = {
        ...
        'conv_user3': {
          userId: 'user3',
          lastMessage: 'Project files ready',
          lastMessageTime: new Date().toISOString(),
          unreadCount: 1
        }
      };
      ...
      function loadConversations() {
        ...
        Object.entries(conversations).forEach(([convId, data]) => {
          ...
          if (user) {
            addConversationToList(convId, user, data);
          }
        });
      }
      ...
      els.photoInput.addEventListener('change', (e) => {
        ...
        if (file) {
          ...
          reader.onload = (event) => {
            const newMsg = {
              id: Date.now(),
              text: `<img src="${event.target.result}" class="max-w-full rounded-lg max-h-64">`,
              sender: 'current',
              time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
              type: 'image'
            };
      ...
      function showProfile() {
        if (!currentUser) {
          ...
          showToast('👤 Profile: ' + currentUser.displayName);
        }
      }
