    ...
    div.innerHTML = `
      ...
      <div class="flex-1 min-w-0">
        <div class="flex justify-between items-center">
          <span class="font-bold truncate">${escapeHtml(user.name)}</span>
          <span class="text-xs opacity-70">${user.lastSeen}</span>
        </div>
        <p class="text-sm opacity-70 truncate">${escapeHtml((data.lastMessage || 'Say hello!').substring(0, 40))}</p>
      </div>
