...
function createContactElement(user) {
  ...
  div.className = `contact-item flex items-center p-4 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 cursor-pointer transition-all mb-2 user-selection ${
    currentUserId === user.id ? "active-user" : ""
  }`;
  ...
  if (message.type === "text") {
    div.innerHTML = `
          ...
          <div class="flex flex-col ${
            message.incoming ? "items-start" : "items-end"
          } max-w-[80%]">
            <div class="${
              message.incoming ? "incoming-message" : "outgoing-message"
            } px-5 py-3">
              ${message.text}
            </div>
