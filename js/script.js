    ...
    (function () {
      ...
      function loadOnlineUsers() {
        ...
        online.forEach(u => {
          ...
          div.innerHTML = `<div class="relative"><div class="avatar avatar-sm" style="background: var(--primary);"><i class="fas fa-${u.avatarIcon}"></i></div><span class="status-badge online"></span></div><span class="text-xs font-medium mt-1 max-w-[64px] truncate">${u.name.split(' ')[0]}</span>`;
      ...
      function loadConversations() {
        ...
        users.forEach(u => {
          ...
          div.onclick = () => selectChat(u.id);
          const favStar = u.isFavorite ? '<i class="fas fa-star text-yellow-400 absolute -top-1 -right-1 text-[9px]"></i>' : '';
          div.innerHTML = `<div class="relative"><div class="avatar avatar-sm" style="background: var(--primary);"><i class="fas fa-${u.avatarIcon}"></i></div><span class="status-badge ${u.status}"></span>${favStar}</div><div class="flex-1 min-w-0"><div class="flex justify-between items-center gap-1"><span class="font-semibold truncate">${u.name}</span><span class="text-xs text-secondary whitespace-nowrap">${u.lastSeen}</span></div><p class="text-sm text-secondary truncate">${u.lastMessage}</p></div>${u.unread ? '<span class="bg-blue-600 text-white text-xs rounded-full px-2 py-0.5 ml-1 shrink-0">' + u.unread + '</span>' : ''}`;
      ...
      function handleFileSelect(input, type) {
        ...
        Array.from(files).forEach(file => {
          ...
          const newMsg = {
            ...
            type: 'attachment',
            content: `<i class="fas ${icon}"></i> ${file.name} (${(file.size / 1024).toFixed(1)} KB)`,
            attachment: {
              name: file.name,
              size: file.size,
              type: file.type,
      ...
      window.addEventListener('resize', () => {
        ...
        if (!isMobile) { closeSidebar(); document.body.style.overflow = ''; }
      });

      // close menus on outside click (basic)
      ...
      window.viewProfile = () => { alert(`view profile ${currentUser?.name}`); els.chatMenu.classList.add('hidden'); };
      window.muteChat = () => { alert('chat muted (backend)'); els.chatMenu.classList.add('hidden'); };
      ...
      window.exportChat = () => { alert('export chat (PDF)'); els.chatMenu.classList.add('hidden'); };
      window.attachPhoto = () => alert('deprecated: use handlePhotoUpload');
      window.attachFile = () => alert('deprecated: use handleFileUpload');
      window.attachLocation = () => alert('deprecated: use handleLocationUpload');
